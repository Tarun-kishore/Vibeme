...
router.post('/resend', async (req, res) => {
    try {
        const user = await User.findOne({ where: { email: req.body.email, verified: false } })

        if (!user) return res.render('IndexPages/notAuth');

        const token = await confirmationToken.findOne({ where: { user: user.id } })
        mail.sendConfirmationMail(`${req.headers.host}/user/verify/${user.id}/${token.token}`,user.email)
