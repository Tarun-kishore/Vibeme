...
router.get('/view/:commentId',async(req,res)=>{
    ...
    try {
        ...
        const comment = await Comment.findByPk(req.params.commentId,{include:[{model:Reply},
        {model:Post}]})
        console.log(comment)
        if(!comment)
            res.render('404',options)

        // const post = await Post.findByPk(comment.commentedOn)
        ...
        res.render('publicPostWithComment',{...options,postData:{...postObject,creator},comments:commentData})
    } catch (e) {
        console.log(e)
...
router.post('/reply/:commentId',auth,async (req,res)=>{
    try {
        const reply = Reply.build({repliedOn : req.params.commentId, repliedBy: req.user.id, comment: req.body.comment,poster: req.user.getFullName()})
