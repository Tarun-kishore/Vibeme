...
router.post('/send/:id',auth,async (req,res)=>{
    ...
    try {
        const connection = await Connection.findOne({where:{
            [Op.or] : [{sentBy : req.user.id , sentTo: req.params.id},{sentBy : req.params.id , sentTo: req.user.id}]}})
        
        if(connection)
            return res.send("Status pending or already in connection")
...
router.get('/view',auth,async(req,res)=>{
    ...
    try {
        options.myConnections = await Connection.findAll({where:{
        ...
        options.sentConnections = await Connection.findAll({where:{
            sentBy : req.user.id,
            pending : true
        },
        include: [{
        ...
        res.render('connections',options)
    } catch (e) {
        console.log(e)
